...
//
//   node engine/shot_kits.mjs --id fog-signal-check
//   node engine/shot_kits.mjs --id opening-the-office --only s2,s3
//   node engine/shot_kits.mjs --peresyom      # все кадры пересъёма
//
// Зачем. Раньше на один кадр уходило: найти лист персонажа в одной папке,
// фон — в другой, скопировать промпт из markdown, а потом переименовать
// «Gemini_Generated_Image_5py4uz…» руками. Восемьдесят процентов времени
// уходило на лазание по папкам.
//
...
// Цепочка: сцена -> от какого ПРИНЯТОГО кадра она продолжается. Принятый
// кадр прикладывается ПОСЛЕДНИМ вложением: нумерация листов не сдвигается,
// а последняя картинка задаёт пропорции выхода. Заполняется вручную по
// production-состоянию текущего ролика; пример формата на демо-серии:
const CHAIN = {
  // "fog-signal-check": { s2: "s1", s3: "s2" },
};
