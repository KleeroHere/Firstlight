...
def raw_copy(target, raw_dir):
    """Path to the ungraded original of `target`, refreshed when `target` is newer.

    Grading is done off the ORIGINAL, not off an already-graded file, so it
    stays idempotent -- run it twice and nothing shifts twice. That means a
    cached copy in `_raw/`. The trap: regenerate the frame (a reshoot after an
    acceptance pass rejected it) and the cache is now older than the frame it
    claims to be the original of. Grading then writes the OLD picture back over
    the new one and silently undoes the reshoot -- money spent, nothing
    changed, and the defect still on screen. So: if the target is newer than
    its cached original, the cache is stale and the target replaces it.
    """
    os.makedirs(raw_dir, exist_ok=True)
    raw_file = os.path.join(raw_dir, os.path.basename(target))
    if not os.path.exists(raw_file) or os.path.getmtime(target) > os.path.getmtime(raw_file) + 1:
        shutil.copy(target, raw_file)
    return raw_file


def match_masters(a):
    """Give every scene one tone.

    Each start frame of a scene is generated by its own image-edit call, and
    two calls off the same background can land a whole grade apart -- one grey
    and hazy, the next a warm sunset. Cut together they read as two different
    evenings. So: the scene's WIDE frame is the reference (it is the one the
    viewer sees first and longest), and every other frame in that scene is
    pulled toward its colour.

    Mean-only by default, not mean+std: a close-up legitimately has different
    contrast from a wide, and matching the full distribution would flatten it.
    Originals are kept beside the corrected files in `_raw/`.
    """
    ...
    raw = os.path.join(takes, "_raw")
    os.makedirs(raw, exist_ok=True)
    only = {int(x) for x in a.only.split(",") if x}
    mode = a.mode if a.mode != "meanstd" else "mean"
    strength = a.strength if a.strength != 1.0 else 0.8

    refs = {}
    for p in plans:
        if (p.get("role") or p.get("shot")) == "wide" and p["scene"] not in refs:
            refs[p["scene"]] = os.path.join(takes, p["master"])
    for p in plans:
        n = p["plan"]
        if only and n not in only:
            continue
        ref = refs.get(p["scene"])
        target = os.path.join(takes, p["master"])
        if not ref or ref == target or not os.path.exists(ref) or not os.path.exists(target):
            continue
        raw_file = raw_copy(target, raw)
        src_img = Image.open(raw_file)
        before = stats(src_img)
        out = match(Image.open(ref), src_img, mode, strength)
        out.save(target)
        print(f"plan {n:>2} {os.path.basename(target):20} {before} -> {stats(out)}  "
              f"scene {p['scene']} ref {os.path.basename(ref)} {stats(Image.open(ref))}")
...
def main():
    ...
    ap.add_argument("--masters", action="store_true",
                    help="match every start frame of a scene to that scene's WIDE frame, "
                         "instead of matching end keys to their master")
    a = ap.parse_args()
    ...
    for p in plans:
        ...
        if not os.path.exists(key_file):
            continue
        raw_file = raw_copy(key_file, raw)
        src_img = Image.open(raw_file)
        before = stats(src_img)
