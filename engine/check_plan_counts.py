...
Найденный на практике брак: правка ключа оставляет старую фразу «В комнате
ровно два человека и больше никого» от прежней редакции сцены, где людей
и правда было двое, а состав сцены с тех пор вырос. Gemini выполняет
инструкцию буквально и стирает лишних из кадра, а следующий план
наследует уже пустое место. То же число живёт и в английских текстах
движения («Exactly five people are in the shot»), и Wan читает его так
же буквально.
...
# Осознанные расхождения — их --fix не трогает. Пример формата (пустой по
# умолчанию — заполняется по конкретному ролику, когда счёт в тексте
# правки намеренно не совпадает со списком chars сцены):
EXCEPT = {
    # ("fog-signal-check", 7, "motion"),
}
