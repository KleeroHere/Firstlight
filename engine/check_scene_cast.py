...
он рисуется «по памяти» — и на экране получается другой человек.
Ровно так уплыли Галя и Ярослав в «Разборе дневников» (приёмка 29.08).
...
reports/scene-cast-audit-29-08.md.
"""
import io
import json
import os
...
def name_stems(name):
    ...
    Основа — имя без последней буквы (покрывает падежи: «Настя» → «Наст» →
    «Настей»); для длинных имён с беглой гласной добавляется основа без двух
    букв («Новичок» → «Нович» → «новичка»). Хвост склонения ограничен тремя
    буквами со границей слова — иначе «Наст» ловил «настенную доску», а
    «Гале» (дат. падеж) не ловился вовсе (ложные строки аудита 30.08).
    """
    first = name.split()[0]
